...
const WINDOWS_DESCRIPTOR = Object.freeze({
...
function validSegments(operation) {
...
function validPathSegments(input) {
    ...
    for (let index = 0; index < input.length; index += 1) {
        ...
        if (segment === '' || segment === '.' || segment === '..'
            ...
            || segment.toLowerCase().startsWith(INTERNAL_NAME_PREFIX))
            return undefined;
        segments.push(segment);
    }
    return pathBytes <= MUTATION_BUDGETS.maxPathBytes ? Object.freeze(segments) : undefined;
}
function validExpected(operation) {
...
function statusIssue(status, phase) {
    ...
    return issue('WORKSPACE_MUTATION_FAILED', `The Windows namespace operation failed (NTSTATUS 0x${code.toString(16).padStart(8, '0')}).`, 409);
}
function aborted() {
    return notCommitted(issue('MUTATION_ABORTED', 'The mutation was cancelled before commit.', 409));
}
function unsupported() {
    return notCommitted(issue('WORKSPACE_MUTATION_UNAVAILABLE', 'This workspace mutation is unavailable on the Windows backend.', 501));
}
function sameNativeIdentity(left, right) {
    return left.volume === right.volume && left.index === right.index;
}
function matchesStat(info, stat) {
...
function nativeKindMatches(info, kind) {
    return ((info.attributes & C.FILE_ATTRIBUTE_DIRECTORY) !== 0) === (kind === 'directory');
}
function sameMovedSnapshot(before, after) {
...
function sameExactPath(left, right) {
    return left.length === right.length && left.every((segment, index) => segment === right[index]);
}
function windowsNameEquivalent(left, right) {
    return left === right
...
class OwnedHandle {
    ...
    closeNative;
    closePromise;
    constructor(generation, value, closeNative) {
...
class WindowsKernel {
    ...
    disposePromise;
    disposing = false;
    constructor(ffi, ntdllLibrary, kernelLibrary, nullPointer) {
        this.ffi = ffi;
        this.ntdllLibrary = ntdllLibrary;
    ...
    static async create() {
        ...
        try {
            ...
            if (!ffi.isNullPointer(nullPointer))
                throw new Error('Could not obtain a typed null pointer.');
            return new WindowsKernel(ffi, ntdllLibrary, kernelLibrary, nullPointer);
        }
        catch (error) {
            if (kernelOpened)
                try {
                    ffi.close(kernelLibrary);
                }
                catch { /* best effort */ }
            if (ntdllOpened)
                try {
                    ffi.close(ntdllLibrary);
                }
                catch { /* best effort */ }
            throw error;
        }
    ...
    openRoot(path) {
        if (this.disposing)
            throw new Error('The native kernel is disposing.');
        ...
        if (fileType !== C.FILE_TYPE_DISK)
            throw new Error('CreateFileW did not return a disk handle.');
    ...
    information(handle) {
        ...
        try {
            ...
            if (nativePointer === undefined)
                throw new Error('Missing file-information pointer.');
            ...
            if (succeeded === 0)
                throw new Error('GetFileInformationByHandle failed.');
    ...
    fileSystemName(handle) {
        ...
        try {
            ...
            throw new IdeHostError('WORKSPACE_MUTATION_UNAVAILABLE', 'The Host cannot verify an NTFS workspace volume.', 501, { cause: error });
        }
        if (succeeded === 0) {
            // This also rejects SMB: Microsoft documents that SMB does not support
            // this volume-management function.
            throw new IdeHostError('WORKSPACE_MUTATION_UNAVAILABLE', 'The Host cannot verify an NTFS workspace volume.', 501);
        }
        let end = 0;
        while (end < nameBuffer.length && nameBuffer.readUInt16LE(end) !== 0)
            end += 2;
        if (end === 0 || end === nameBuffer.length) {
            throw new IdeHostError('WORKSPACE_MUTATION_UNAVAILABLE', 'The Host returned an invalid workspace filesystem name.', 501);
        }
        return nameBuffer.subarray(0, end).toString('utf16le');
    }
    normalizedLeafBytes(handle) {
        const t = this.ffi.DataType;
    ...
    attributeTagInformation(handle) {
        ...
        if (succeeded === 0)
            throw new Error('FileAttributeTagInfo failed.');
        return { attributes: buffer.readUInt32LE(0), reparseTag: buffer.readUInt32LE(4) };
    }
    ntOpenDirectory(parent, segment) {
        return this.ntOpen(parent, segment, C.FILE_OPEN, C.FILE_ATTRIBUTE_DIRECTORY, C.FILE_DIRECTORY_FILE | C.FILE_OPEN_REPARSE_POINT | C.FILE_SYNCHRONOUS_IO_NONALERT, C.FILE_LIST_DIRECTORY | C.FILE_TRAVERSE | C.FILE_READ_ATTRIBUTES | C.SYNCHRONIZE);
    }
    ntOpenDeleteDirectory(parent, segment) {
        return this.ntOpen(parent, segment, C.FILE_OPEN, C.FILE_ATTRIBUTE_DIRECTORY, C.FILE_DIRECTORY_FILE | C.FILE_OPEN_REPARSE_POINT | C.FILE_SYNCHRONOUS_IO_NONALERT, C.FILE_LIST_DIRECTORY | C.FILE_TRAVERSE | C.FILE_READ_ATTRIBUTES | C.SYNCHRONIZE, C.FILE_SHARE_READ);
    }
    ntCreateLeaf(parent, segment, kind) {
        const directory = kind === 'directory';
        return this.ntOpen(parent, segment, C.FILE_CREATE, directory ? C.FILE_ATTRIBUTE_DIRECTORY : C.FILE_ATTRIBUTE_NORMAL, directory
            ? C.FILE_DIRECTORY_FILE | C.FILE_OPEN_REPARSE_POINT | C.FILE_SYNCHRONOUS_IO_NONALERT
            : C.FILE_NON_DIRECTORY_FILE | C.FILE_OPEN_REPARSE_POINT | C.FILE_SYNCHRONOUS_IO_NONALERT, (directory ? C.FILE_LIST_DIRECTORY | C.FILE_TRAVERSE : 0)
            | C.FILE_READ_ATTRIBUTES | C.SYNCHRONIZE);
    }
    ntOpenRenameSource(parent, segment, kind) {
        const directory = kind === 'directory';
        return this.ntOpen(parent, segment, C.FILE_OPEN, directory ? C.FILE_ATTRIBUTE_DIRECTORY : C.FILE_ATTRIBUTE_NORMAL, (directory ? C.FILE_DIRECTORY_FILE : C.FILE_NON_DIRECTORY_FILE)
            | C.FILE_OPEN_REPARSE_POINT
            | C.FILE_SYNCHRONOUS_IO_NONALERT, C.DELETE | C.FILE_READ_ATTRIBUTES | C.SYNCHRONIZE);
    }
    ntOpenDeleteSource(parent, segment, kind) {
        const directory = kind === 'directory';
        return this.ntOpen(parent, segment, C.FILE_OPEN, directory ? C.FILE_ATTRIBUTE_DIRECTORY : C.FILE_ATTRIBUTE_NORMAL, (directory ? C.FILE_DIRECTORY_FILE : C.FILE_NON_DIRECTORY_FILE)
            | C.FILE_OPEN_REPARSE_POINT | C.FILE_SYNCHRONOUS_IO_NONALERT, C.DELETE | C.FILE_READ_ATTRIBUTES | C.FILE_WRITE_ATTRIBUTES | C.SYNCHRONIZE
            | (directory ? C.FILE_LIST_DIRECTORY | C.FILE_TRAVERSE : 0), C.FILE_SHARE_READ);
    }
    ntOpenDeleteRaw(parent, name, kind) {
        const directory = kind === 'directory';
        return this.ntOpenBytes(parent, name, C.FILE_OPEN, directory ? C.FILE_ATTRIBUTE_DIRECTORY : C.FILE_ATTRIBUTE_NORMAL, (directory ? C.FILE_DIRECTORY_FILE : C.FILE_NON_DIRECTORY_FILE)
            | C.FILE_OPEN_REPARSE_POINT | C.FILE_SYNCHRONOUS_IO_NONALERT, C.DELETE | C.FILE_READ_ATTRIBUTES | C.FILE_WRITE_ATTRIBUTES | C.SYNCHRONIZE
            | (directory ? C.FILE_LIST_DIRECTORY | C.FILE_TRAVERSE : 0), C.FILE_SHARE_READ);
    }
    ntProbeAbsentRaw(parent, name, kind) {
        return this.ntOpenBytes(parent, name, C.FILE_OPEN, kind === 'directory' ? C.FILE_ATTRIBUTE_DIRECTORY : C.FILE_ATTRIBUTE_NORMAL, (kind === 'directory' ? C.FILE_DIRECTORY_FILE : C.FILE_NON_DIRECTORY_FILE)
            | C.FILE_OPEN_REPARSE_POINT | C.FILE_SYNCHRONOUS_IO_NONALERT, C.FILE_READ_ATTRIBUTES | C.SYNCHRONIZE, C.FILE_SHARE_READ);
    }
    ntOpenProbeReader(parent, segment) {
        return this.ntOpen(parent, segment, C.FILE_OPEN, C.FILE_ATTRIBUTE_NORMAL, C.FILE_NON_DIRECTORY_FILE | C.FILE_OPEN_REPARSE_POINT | C.FILE_SYNCHRONOUS_IO_NONALERT, C.FILE_READ_DATA | C.FILE_READ_ATTRIBUTES | C.SYNCHRONIZE, C.FILE_SHARE_READ | C.FILE_SHARE_WRITE | C.FILE_SHARE_DELETE);
    }
    readProbe(handle, length) {
        const t = this.ffi.DataType;
        ...
        try {
            ...
            if (countNative === undefined)
                throw new Error('Missing ReadFile count pointer.');
            ...
            if (succeeded === 0)
                throw new Error('ReadFile failed for the POSIX-delete witness.');
            const restored = this.ffi.restorePointer({ retType: [countType], paramsValue: count });
    ...
    ntDeletePosix(handle) {
        ...
        try {
            ...
            if (informationNative === undefined || ioNative === undefined)
    ...
    queryDirectory(handle, restart, firstOnly = false) {
        ...
        if (succeeded === 0) {
            ...
            if (error === C.ERROR_NO_MORE_FILES)
                return { entries: [], noMoreFiles: true };
            throw new Error(`Directory enumeration failed with Win32 error ${error}.`);
        }
        const entries = [];
        let offset = 0;
        for (let guard = 0; guard < 65_536; guard += 1) {
            if (offset + 88 > buffer.length)
                throw new Error('Truncated directory record.');
    ...
    ntRenameNoReplace(source, destinationParent, destinationLeaf) {
        if (this.disposing)
            throw new Error('The native kernel is disposing.');
        const t = this.ffi.DataType;
        ...
        try {
            ...
            if (renameNative === undefined)
                throw new Error('Missing FILE_RENAME_INFORMATION pointer.');
            ...
            if (ioNative === undefined)
                throw new Error('Missing rename IO_STATUS_BLOCK pointer.');
            return this.ffi.load({
                library: this.ntdllLibrary,
                funcName: 'NtSetInformationFile',
                retType: t.I32,
            ...
            if (errors.length > 0)
                throw new AggregateError(errors, 'Could not free native rename parameters.');
        }
    }
    dispose() {
        this.disposePromise ??= (async () => {
            ...
            try {
                this.ffi.close(this.ntdllLibrary);
            }
            catch { /* best effort after handles drain */ }
            try {
                this.ffi.close(this.kernelLibrary);
            }
            catch { /* best effort after handles drain */ }
    ...
    ntOpen(parent, segment, disposition, attributes, options, desiredAccess, shareAccess = C.FILE_SHARE_READ | C.FILE_SHARE_WRITE) {
        return this.ntOpenBytes(parent, Buffer.from(segment, 'utf16le'), disposition, attributes, options, desiredAccess, shareAccess);
    }
    ntOpenBytes(parent, name, disposition, attributes, options, desiredAccess, shareAccess) {
        if (this.disposing)
            throw new Error('The native kernel is disposing.');
        if (name.length === 0 || name.length > 510 || name.length % 2 !== 0) {
        ...
        try {
            ...
            if (nameBuffer === undefined)
                throw new Error('Missing relative-name pointer.');
            ...
            if (unicodeNative === undefined)
                throw new Error('Missing UNICODE_STRING pointer.');
            ...
            if (attributesNative === undefined)
                throw new Error('Missing OBJECT_ATTRIBUTES pointer.');
            ...
            const ioNative = this.ffi.unwrapPointer(ioPointer)[0];
            if (ioNative === undefined)
                throw new Error('Missing IO_STATUS_BLOCK pointer.');
            ...
            if (this.ffi.isNullPointer(value)) {
                if (status < 0)
                    return { status, information, ambiguous: false };
            ...
            if (errors.length > 0)
                throw new AggregateError(errors, 'Could not free native FFI parameters.');
        }
    }
}
class WindowsMutationWorkspace {
    workspaceId;
    registeredRoot;
    root;
    rootInformation;
    rootFileIdInformation;
    kernel;
    onDispose;
    executions = new Set();
    disposePromise;
    disposed = false;
    constructor(workspaceId, registeredRoot, root, rootInformation, rootFileIdInformation, kernel, onDispose) {
        this.workspaceId = workspaceId;
        this.registeredRoot = registeredRoot;
        this.root = root;
        this.rootInformation = rootInformation;
        this.rootFileIdInformation = rootFileIdInformation;
        this.kernel = kernel;
        ...
        try {
            for (let index = 0; index < segments.length - 1; index += 1) {
                ...
                if (this.kernel.normalizedLeafName(opened.handle) !== segment) {
                    return notCommitted(issue('PATH_IDENTITY_CHANGED', 'A workspace parent resolved to a different normalized name.', 409));
                }
            ...
            if (created.information !== C.FILE_CREATED)
                return recovery();
            if (this.kernel.normalizedLeafName(created.handle) !== leafName)
                return recovery();
            ...
            if (closeErrors.length > 0)
                throw new AggregateError(closeErrors, 'Could not close native mutation handles.');
        }
    }
    consumeDeleteQuery(budget) {
        budget.queries += 1;
        if (budget.queries > DELETE_MAX_QUERIES)
            throw new Error('The recursive-delete scan budget was exceeded.');
    }
    consumeDeleteEntry(budget, entry) {
        if (!safeRawLeaf(entry.name))
            throw new Error('A directory returned an unsafe raw leaf name.');
        budget.entries += 1;
    ...
    readAllDeleteEntries(directory, budget) {
        const entries = [];
        let restart = true;
        for (;;) {
    ...
    readFirstDeleteEntry(directory, budget) {
        let restart = true;
        for (;;) {
            this.consumeDeleteQuery(budget);
    ...
    async openVerifiedDeleteEntry(parent, entry) {
        ...
        if (opened.ambiguous || opened.status !== 0 || opened.handle === undefined) {
            if (opened.handle !== undefined) {
                try {
                    ...
                    throw new NativeCloseError(error);
                }
            }
            if (opened.ambiguous)
                throw new NativeDeleteAmbiguousError('A recursive child open was ambiguous.');
        ...
        try {
            ...
            try {
                ...
                throw new NativeCloseError(closeError);
            }
            throw error;
        }
    }
    async preflightDeleteTree(directory, depth, budget, signal) {
        if (depth > DELETE_MAX_DEPTH)
            throw new Error('The recursive-delete depth budget was exceeded.');
        if (signal.aborted)
            throw new Error('The recursive delete was aborted during preflight.');
    ...
    async assertDeleteNameAbsent(parent, rawName, kind) {
    ...
    async purgeDeleteTree(directory, depth, budget) {
        if (depth > DELETE_MAX_DEPTH)
            throw new Error('The recursive-delete depth budget was exceeded.');
        for (;;) {
            ...
            try {
                ...
                for (;;) {
                    ...
                    if (unsignedStatus(status) === STATUS.DIRECTORY_NOT_EMPTY
                        && entry.kind === 'directory' && !entry.reparsePoint)
                        continue;
                    break;
                }
                if (status !== 0)
                    throw new Error(`Recursive disposition failed (0x${unsignedStatus(status).toString(16)}).`);
                try {
                    await opened.handle.close();
                }
                catch (error) {
    ...
    async delete(operation, signal) {
        ...
        try {
            outcome = await (async () => {
                ...
                if ('outcome' in parentResult)
                    return parentResult.outcome;
                ...
                let firstSnapshot;
                try {
                    firstSnapshot = await lstat(sourcePath, { bigint: true });
                }
                catch {
                    return notCommitted(issue('SOURCE_IDENTITY_CHANGED', 'The source resource changed before commit.', 409));
                }
                ...
                if (operation.recursive && operation.expected.kind === 'directory') {
                    await this.preflightDeleteTree(source, 0, { entries: 0, queries: 0, nameBytes: 0 }, signal);
                }
                const fenceInformation = this.kernel.information(source);
                let fenceSnapshot;
                try {
                    fenceSnapshot = await lstat(sourcePath, { bigint: true });
                }
                catch {
                    return notCommitted(issue('SOURCE_IDENTITY_CHANGED', 'The source resource changed before commit.', 409));
                }
                ...
                if (operation.recursive && operation.expected.kind === 'directory') {
                    ...
                    if (!this.kernel.normalizedLeafBytes(source).equals(tombstoneRaw))
                        return recovery();
                    const movedInformation = this.kernel.information(source);
                    if (!sameNativeIdentity(firstInformation, movedInformation)
                        || movedInformation.attributes !== firstInformation.attributes)
                        return recovery();
                    await this.assertDeleteNameAbsent(parentResult.parent, sourceRaw, 'directory');
                    const purgeBudget = { entries: 0, queries: 0, nameBytes: 0 };
                    let rootStatus;
                    for (;;) {
                        await this.purgeDeleteTree(source, 0, purgeBudget);
                        rootStatus = this.kernel.ntDeletePosix(source);
                        if (unsignedStatus(rootStatus) !== STATUS.DIRECTORY_NOT_EMPTY)
                            break;
                    }
                    if (rootStatus !== 0)
                        throw new Error(`Root disposition failed (0x${unsignedStatus(rootStatus).toString(16)}).`);
                    try {
                        await source.close();
                    }
                    catch (error) {
        ...
        if (closeErrors.length > 0)
            return recovery(new AggregateError(closeErrors, 'Could not close native delete handles.'));
        return outcome ?? recovery();
    }
    openDeleteParent(segments, signal, held) {
        let parent = this.root;
        for (let index = 0; index < segments.length - 1; index += 1) {
            if (signal.aborted)
    ...
    async rename(operation, signal) {
        ...
        if (operation.expected.kind === 'directory'
            ...
            return notCommitted(issue('INVALID_DESTINATION', 'A directory cannot be renamed into its own subtree.', 400));
        }
        if (signal.aborted)
            return aborted();
        const held = [];
        ...
        try {
            ...
            if ('outcome' in sourceParentResult)
                return sourceParentResult.outcome;
            const destinationParentResult = this.openRenameParent(destinationSegments, signal, held);
            if ('outcome' in destinationParentResult)
                return destinationParentResult.outcome;
            if (signal.aborted)
                return aborted();
            ...
            const openedSource = this.kernel.ntOpenRenameSource(sourceParentResult.parent, sourceLeaf, operation.expected.kind);
            if (openedSource.handle !== undefined)
                held.push(openedSource.handle);
            if (openedSource.ambiguous)
                return recovery();
            ...
            if (this.kernel.normalizedLeafName(openedSource.handle) !== sourceLeaf) {
                return notCommitted(issue('PATH_IDENTITY_CHANGED', 'The source resolved to a different normalized name.', 409));
            }
            ...
            if (observedKind === undefined) {
                return notCommitted(issue('RESOURCE_TYPE_UNSUPPORTED', 'Only regular files and directories can be renamed.', 409));
            }
            ...
            if (signal.aborted)
                return aborted();
            commitEntered = true;
            const status = this.kernel.ntRenameNoReplace(openedSource.handle, destinationParentResult.parent, destinationLeaf);
            if (status !== 0) {
                if (status < 0) {
                    commitEntered = false;
            ...
            if (this.kernel.normalizedLeafName(openedSource.handle) !== destinationLeaf)
                return recovery();
            ...
            if (!sameNativeIdentity(sourceInformation, committedInformation)
                ...
                || !nativeKindMatches(committedInformation, operation.expected.kind))
                return recovery();
            ...
            if (!sameNativeIdentity(committedInformation, finalInformation)
                ...
                || !sameMovedSnapshot(sourceSnapshot, destinationSnapshot))
                return recovery();
            return {
                state: 'committed',
                evidence: {
            ...
            return commitEntered
                ? recovery(error)
                : notCommitted(issue('WORKSPACE_MUTATION_FAILED', 'The Host could not prepare the workspace rename.', 409));
        }
        finally {
            const closeErrors = [];
            for (let index = held.length - 1; index >= 0; index -= 1) {
            ...
            if (closeErrors.length > 0)
                throw new AggregateError(closeErrors, 'Could not close native rename handles.');
        }
    }
    openRenameParent(segments, signal, held) {
        let parent = this.root;
        for (let index = 0; index < segments.length - 1; index += 1) {
            if (signal.aborted)
                return { outcome: aborted() };
            ...
            if ((information.attributes & C.FILE_ATTRIBUTE_REPARSE_POINT) !== 0) {
                return { outcome: notCommitted(issue('SYMLINK_NOT_ALLOWED', 'Reparse points are not available in the IDE.', 403)) };
            }
            if ((information.attributes & C.FILE_ATTRIBUTE_DIRECTORY) === 0) {
                return { outcome: notCommitted(issue('NOT_DIRECTORY', 'A workspace parent path is not a directory.', 400)) };
            }
            if (information.volume !== this.rootInformation.volume) {
                return { outcome: notCommitted(issue('MOUNT_NOT_ALLOWED', 'Mounted workspace paths are not available in the IDE.', 403)) };
            }
            if (this.kernel.normalizedLeafName(opened.handle) !== segment) {
                return { outcome: notCommitted(issue('PATH_IDENTITY_CHANGED', 'A workspace parent resolved to a different normalized name.', 409)) };
            }
            parent = opened.handle;
        }
        return { parent };
...
class WindowsMutationBackend {
    kernel;
    descriptor = WINDOWS_DESCRIPTOR;
    workspaces = new Set();
    opens = new Set();
    disposePromise;
    disposed = false;
    ...
    openWorkspace(request) {
        if (this.disposed) {
            return Promise.reject(new IdeHostError('WORKSPACE_MUTATION_UNAVAILABLE', 'The Windows workspace mutation backend is disposed.', 501));
        }
    ...
    async openWorkspaceAdmitted(request) {
        ...
        try {
            ...
            if (this.kernel.fileSystemName(root) !== 'NTFS') {
                throw new IdeHostError('WORKSPACE_MUTATION_UNAVAILABLE', 'Containment-safe workspace mutations currently require a local NTFS volume.', 501);
            }
            ...
            const workspace = new WindowsMutationWorkspace(request.workspaceId, request.registeredRoot, root, information, this.kernel.fileIdInformation(root), this.kernel, disposed => { this.workspaces.delete(disposed); });
...
async function probe(kernel) {
    ...
    try {
        ...
        workspace = new WindowsMutationWorkspace('probe', rootPath, probeRoot, information, kernel.fileIdInformation(probeRoot), kernel, () => { });
        probeRoot = undefined;
 ...
 */
export async function createWindowsMutationBackend() {
