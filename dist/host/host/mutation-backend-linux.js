...
const LINUX_DESCRIPTOR = Object.freeze({
...
function errnoIssue(errnoCode, phase) {
    ...
    if (errnoCode === 40)
        return issue('SYMLINK_NOT_ALLOWED', 'Symbolic links are not available in the IDE.', 403);
    if (errnoCode === 18)
        return issue('CROSS_DEVICE_MUTATION', 'Cross-device workspace mutations are not allowed.', 409);
...
class LinuxKernel {
    ...
    static async create() {
        ...
        try {
            ...
            if (childFd < 0)
                throw new Error('openat2 is unavailable.');
    ...
    async openAt2(parentFd, relative, flags, mode) {
        ...
        const result = await this.call('syscall', this.ffi.DataType.I64, [this.ffi.DataType.I64, this.ffi.DataType.I64, this.ffi.DataType.String,
            this.ffi.DataType.U8Array, this.ffi.DataType.U64], [C.SYS_OPENAT2, parentFd, relative, how, how.length]);
        return result.value < 0 ? -result.errnoCode : result.value;
    }
    async mkdirAt(parentFd, name) {
        return await this.call('mkdirat', this.ffi.DataType.I32, [this.ffi.DataType.I32, this.ffi.DataType.String, this.ffi.DataType.U32], [parentFd, name, 0o777]);
    }
    async snapshot(fd) {
        if (fd < 0)
            throw new Error('Cannot inspect an invalid Linux file descriptor.');
    ...
    async call(funcName, retType, paramsType, paramsValue) {
        if (this.disposed)
            throw new Error('The Linux native kernel is disposed.');
...
class LinuxMutationWorkspace {
    ...
    disposed = false;
    poisoned = false;
    constructor(workspaceId, registeredRoot, rootFd, rootSnapshot, kernel, onDisposed) {
    ...
    execute(request) {
        if (this.disposed)
            return Promise.resolve(notCommitted(issue('WORKSPACE_MUTATION_UNAVAILABLE', 'The Linux workspace mutation backend is disposed.', 501)));
        if (this.poisoned)
    ...
    async executeSerial(request) {
        ...
        try {
            ...
            return notCommitted(issue('WORKSPACE_MUTATION_UNAVAILABLE', 'Containment-safe rename and delete are unavailable on this Linux Host.', 501));
        }
        catch {
            this.poisoned = true;
            return recovery();
    ...
    async create(operation, signal) {
        ...
        try {
            ...
            if (operation.kind === 'createFile') {
                ...
                createdFd = await this.kernel.openAt2(this.rootFd, segments.join('/'), C.O_WRONLY | C.O_CREAT | C.O_EXCL | C.O_NOFOLLOW | C.O_CLOEXEC, 0o666);
                if (createdFd < 0)
                    return notCommitted(errnoIssue(-createdFd, 'create'));
            }
            else {
                const result = await this.kernel.mkdirAt(parent.fd, leaf);
                if (result.value !== 0)
                    return notCommitted(errnoIssue(result.errnoCode, 'create'));
            ...
            if (kindOf(snapshot) !== expectedKind)
                return recovery();
            const observed = await lstat(join(this.registeredRoot, ...segments), { bigint: true });
            if (!sameFileIdentity(snapshot, observed) || kindOf(observed) !== expectedKind)
                return recovery();
            return {
                state: 'committed',
    ...
    async openParent(segments) {
        ...
        try {
            ...
            if (!snapshot.isDirectory() || snapshot.dev !== this.rootSnapshot.dev) {
                ...
                return { outcome: notCommitted(issue('PARENT_IDENTITY_CHANGED', 'A workspace parent directory changed before commit.', 409)) };
            }
            return { fd };
        }
        catch {
...
class LinuxMutationBackend {
    kernel;
    descriptor = LINUX_DESCRIPTOR;
    workspaces = new Set();
    opens = new Set();
    disposePromise;
    disposed = false;
    ...
    openWorkspace(request) {
        if (this.disposed)
            return Promise.reject(new IdeHostError('WORKSPACE_MUTATION_UNAVAILABLE', 'The Linux workspace mutation backend is disposed.', 501));
    ...
    async openWorkspaceAdmitted(request) {
        ...
        try {
            ...
            if (request.signal.aborted || this.disposed)
                throw new IdeHostError('WORKSPACE_MUTATION_UNAVAILABLE', 'Workspace opening was cancelled.', 409);
            const workspace = new LinuxMutationWorkspace(request.workspaceId, request.registeredRoot, rootFd, rootSnapshot, this.kernel, disposed => { this.workspaces.delete(disposed); });
 ...
 */
export async function createLinuxMutationBackend() {
