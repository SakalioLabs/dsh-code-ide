...
export function createStaticHandler(staticRoot, routePrefix) {
    const root = realpathSync(staticRoot);
    const index = realpathSync(resolve(root, 'index.html'));
    if (!inside(root, index) || !statSync(index).isFile())
        throw new Error('dsh-code-ide: staticRoot has no index.html');
    return async (request, response, pathname) => {
        ...
        if (segments.length > 0) {
            ...
            if (!inside(root, unresolved))
                throw new IdeHostError('INVALID_STATIC_PATH', 'Static path escapes its root.', 403);
            try {
                const candidate = realpathSync(unresolved);
                if (inside(root, candidate) && statSync(candidate).isFile())
                    file = candidate;
                else if (extname(segments.at(-1) ?? '') !== '')
                    throw new IdeHostError('ASSET_NOT_FOUND', 'Static asset not found.', 404);
            }
            catch (error) {
                if (error instanceof IdeHostError)
                    throw error;
                if (extname(segments.at(-1) ?? '') !== '')
                    throw new IdeHostError('ASSET_NOT_FOUND', 'Static asset not found.', 404);
            }
        }
