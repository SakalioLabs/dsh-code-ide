...
import { FfiNativeRenameAdapter } from './native-rename.js';
import { createUnavailableMutationBackend, } from './mutation-backend.js';
import { assertNoNestedMount, parseWorkspacePath, resolveWorkspacePath, resolveWorkspaceRoot, } from './path-policy.js';
...
function requireExactKeys(record, required, label) {
    if (required.some(key => !Object.hasOwn(record, key))) {
        throw new IdeHostError('INVALID_REQUEST', `${label} is missing a required field.`);
    }
}
function boundedString(value, maxBytes, code, label) {
...
function validBackendVersion(value) {
...
function uuid(value, label) {
    ...
    if (!UUID.test(parsed))
        throw new IdeHostError('INVALID_OPERATION_ID', `${label} must be a canonical UUID.`);
    return parsed;
}
function epoch(value) {
    const parsed = boundedString(value, MUTATION_BUDGETS.maxProviderEpochBytes, 'INVALID_PROVIDER_EPOCH', 'providerEpoch');
    if (!UUID.test(parsed))
        throw new IdeHostError('INVALID_PROVIDER_EPOCH', 'providerEpoch must be a canonical UUID.');
    return parsed;
}
function expectedResource(value) {
    ...
    if (!VERSION.test(version))
        throw new IdeHostError('INVALID_VERSION', 'expected.version is not a valid opaque token.');
...
function mutationOf(value) {
    ...
    if (!Object.hasOwn(base, 'kind'))
        throw new IdeHostError('INVALID_REQUEST', 'mutation.kind is required.');
    switch (base.kind) {
        case 'createFile': {
        ...
        case 'rename': {
            ...
            if (path === destinationPath)
                throw new IdeHostError('INVALID_DESTINATION', 'The destination must differ from the source.');
...
export function parseWorkspaceMutationRequest(value) {
    const base = exactRecord(value, ['op', 'providerEpoch', 'operationId', 'workspaceId', 'mutation'], 'request');
    if (!Object.hasOwn(base, 'op'))
        throw new IdeHostError('INVALID_REQUEST', 'op is required.');
    if (base.op === 'provider') {
    ...
    if (base.op === 'mutate') {
        ...
        return {
            ...
            workspaceId: boundedString(record.workspaceId, MUTATION_BUDGETS.maxWorkspaceIdBytes, 'INVALID_WORKSPACE_ID', 'workspaceId'),
            mutation: mutationOf(record.mutation),
        };
    }
    throw new IdeHostError('UNKNOWN_OPERATION', 'Unknown workspace mutation operation.');
}
function wireParent(path) {
    const slash = path.lastIndexOf('/');
    return slash === -1 ? '' : path.slice(0, slash);
}
function refreshDirectories(...paths) {
...
function assertExpected(info, expected) {
    const kind = kindOf(info);
    if (kind === undefined)
        throw new IdeHostError('RESOURCE_TYPE_UNSUPPORTED', 'Only regular files and directories can be mutated.', 409);
    if (kind !== expected.kind)
        throw new IdeHostError('RESOURCE_KIND_CONFLICT', 'The resource kind changed.', 409);
    if (versionOf(info) !== expected.version)
        throw new IdeHostError('VERSION_CONFLICT', 'The resource changed before the mutation.', 409);
}
...
function assertMovedSnapshot(expected, actual) {
...
function assertUnchangedSnapshot(expected, actual) {
    if (!sameFileSnapshot(expected, actual) || kindOf(expected) !== kindOf(actual)) {
        throw new RecoveryRequiredError();
    }
}
async function captureDirectoryFence(root, wirePath) {
...
async function assertDirectoryFence(root, fence, options) {
...
async function purgeTree(path, expected, workspaceRoot, maxEntries) {
    ...
    const remove = async (target) => {
        ...
        if (entries > maxEntries)
            throw new Error('quarantine purge budget exceeded');
        ...
        try {
            ...
            for await (const child of directory)
                await remove(join(target, child.name));
        }
        finally {
...
class TombstonePurger {
    ...
    timerDue;
    disposed = false;
    constructor(maxJobs, maxEntries, now, logger) {
    ...
    enqueueReserved(job) {
        if (this.reservations <= 0)
            throw new Error('missing purge reservation');
        this.reservations -= 1;
        this.jobs.push(job);
        this.schedule();
    }
    async dispose() {
        ...
        while (this.jobs.length > 0)
            await this.runOne(true);
    }
    schedule() {
        if (this.disposed || this.running !== undefined || this.jobs.length === 0)
            return;
        ...
        if (this.timer !== undefined && this.timerDue !== undefined) {
            if (this.timerDue <= first.notBefore)
                return;
            clearTimeout(this.timer);
        }
...
export class WorkspaceMutationService {
    ...
    logger;
    internals;
    providerEpoch = randomUUID();
    nativeRename;
    backend;
    ownsBackend;
    backendWorkspaces = new Map();
    backendLifetime = new AbortController();
    now;
    receiptTtlMs;
    maxReceipts;
    maxOperationIds;
    ...
    purger;
    accepting = true;
    constructor(resources, logger, options = {}, internals = {}, backend) {
        ...
        this.purger = new TombstonePurger(positiveBound(options.maxPurgeJobs, 1_024, 65_536, 'maxMutationPurgeJobs'), positiveBound(options.maxPurgeEntries, 100_000, 1_000_000, 'maxMutationPurgeEntries'), this.now, logger);
    }
    async request(value, signal) {
        const request = parseWorkspaceMutationRequest(value);
        if (request.op === 'provider')
    ...
    status(operationId) {
        ...
        if (receipt !== undefined)
            return copyReceipt(receipt.snapshot);
    ...
    async dispose() {
        ...
        for (const receipt of this.receipts.values()) {
            if (receipt.snapshot.state === 'queued' || receipt.snapshot.state === 'running')
    ...
    async mutate(request, signal) {
        ...
        if (this.seen.size >= this.maxOperationIds) {
            throw new IdeHostError('MUTATION_IDEMPOTENCY_CAPACITY', 'Mutation idempotency capacity is exhausted; reconnect after the provider restarts.', 429);
        }
        ...
        const transportAbort = () => { controller.abort(); };
        if (signal?.aborted)
            controller.abort();
        else
    ...
    async executeReceipt(receipt, workspace, mutation) {
        try {
                ...
                try {
                    this.purger.enqueueReserved({
                        ...
                        onFailure: (willRetry) => {
                            ...
                            if (receipt.snapshot.state !== 'committed')
                                return;
    ...
    async execute(workspace, mutation, signal) {
    ...
    async backendWorkspace(workspace) {
        ...
        if (pending === undefined) {
            ...
            void pending.catch(() => {
                if (this.backendWorkspaces.get(workspaceId) === pending)
                    this.backendWorkspaces.delete(workspaceId);
            });
        }
        return await pending;
    }
    async executeBackend(workspace, mutation, signal) {
        ...
        if (outcome.state !== 'committed') {
            ...
            if (outcome.state === 'recoveryRequired')
                throw new RecoveryRequiredError({ cause: issue });
            throw issue;
        }
        const evidence = outcome.evidence;
        if (evidence.kind !== mutation.kind)
            throw new RecoveryRequiredError();
        ...
        switch (mutation.kind) {
            case 'createFile': {
                if (evidence.kind !== 'createFile' || evidence.resourceKind !== 'file'
                    || !validBackendVersion(evidence.version))
                    throw new RecoveryRequiredError();
                return {
                    result: {
                        kind: 'file',
            ...
            case 'delete': {
                if (evidence.kind !== 'delete' || evidence.resourceKind !== mutation.expected.kind
                    || evidence.recursive !== mutation.recursive)
                    throw new RecoveryRequiredError();
                return {
                    result: {
                        kind: evidence.resourceKind,
    ...
    async createFile(workspace, path, signal) {
        const root = await this.resources.resolveRoot(workspace);
        ...
        try {
            ...
            if (published && !(error instanceof RecoveryRequiredError))
                throw new RecoveryRequiredError({ cause: error });
            throw error;
        }
        finally {
            ...
            if (stagingPresent && !published)
                await unlink(staging).catch(() => { });
        }
    }
    async createDirectory(workspace, path, signal) {
        ...
        try {
            ...
            if (created && !(error instanceof RecoveryRequiredError))
                throw new RecoveryRequiredError({ cause: error });
            throw error;
        }
    }
    async rename(workspace, mutation, signal) {
    ...
    async delete(workspace, mutation, signal) {
        if (!await this.nativeRename.supported()) {
            throw new IdeHostError('ATOMIC_RENAME_UNSUPPORTED', 'Atomic no-replace rename is unavailable on this Host.', 501);
        }
        let root = await this.resources.resolveRoot(workspace);
    ...
    async directoryHasEntries(path) {
        const handle = await open(path, READ_DIRECTORY_FLAGS);
        try {
            const info = await handle.stat({ bigint: true });
            if (!info.isDirectory())
                throw new IdeHostError('RESOURCE_KIND_CONFLICT', 'The directory kind changed.', 409);
        }
        finally {
            await handle.close();
        }
    ...
    assertCapability(kind) {
        ...
        if (!backend[kind]) {
            throw new IdeHostError('ATOMIC_RENAME_UNSUPPORTED', 'A containment-safe atomic mutation backend is unavailable on this Host.', 501);
        }
        return this.internals.backendCapabilities === undefined ? undefined : this.assertNativeCapability();
    }
    async assertNativeCapability() {
        if (!await this.nativeRename.supported()) {
            throw new IdeHostError('ATOMIC_RENAME_UNSUPPORTED', 'A containment-safe atomic mutation backend is unavailable on this Host.', 501);
        }
    }
    ...
    sweep() {
        ...
        for (const [id, receipt] of this.receipts) {
            if (receipt.expiresAt <= now && !receipt.purgePending)
