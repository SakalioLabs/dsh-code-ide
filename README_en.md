...
- Full create, move/rename, and permanent-delete support for local NTFS workspaces on Windows x64. Linux Hosts that pass the runtime `openat2` probe, and local APFS workspaces on macOS x64/arm64 that pass the runtime `libSystem` probe, support file and folder creation only.
