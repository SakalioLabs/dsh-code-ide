...
import type { FieldType } from 'ffi-rs'
...
const DARWIN_DESCRIPTOR: Readonly<MutationBackendDescriptor> = Object.freeze({
...
function nativeResult(value: unknown): DarwinNativeResult {
  ...
  return { value: result.value as number, errnoCode: result.errnoCode as number }
}
...
function unsupported(): MutationBackendOutcome {
  return notCommitted(issue(
    ...
    'This structural workspace operation is unavailable on this Host.',
    501,
  ))
}
...
function validPathSegments(input: unknown): readonly string[] | undefined {
  ...
  for (const value of input) {
    ...
    if (value === '' || value === '.' || value === '..'
      ...
      || isInternalWorkspaceName(value)) return undefined
    segments.push(value)
  }
...
function expectedCreatedKind(info: BigIntStats, directory: boolean): boolean {
  return !info.isSymbolicLink()
    && (directory ? info.isDirectory() : info.isFile())
}

function errnoIssue(code: number, phase: 'parent' | 'create' | 'publish'): MutationBackendIssue {
  if (code === E.EEXIST && (phase === 'create' || phase === 'publish')) {
    return issue('DESTINATION_EXISTS', 'The destination already exists.', 409)
  }
  if (code === E.ENOENT) return issue('NOT_FOUND', 'A workspace parent directory no longer exists.', 404)
  if (code === E.ENOTDIR) return issue('NOT_DIRECTORY', 'A workspace parent path is not a directory.', 400)
  if (code === E.ELOOP) return issue('SYMLINK_NOT_ALLOWED', 'Symbolic links are not available in IDE mutation paths.', 403)
  if (code === E.EXDEV) return issue('MOUNT_NOT_ALLOWED', 'Mounted workspace paths are not available in the IDE.', 403)
  if (code === E.EACCES || code === E.EPERM || code === E.EROFS) {
    return issue('PERMISSION_DENIED', 'The Host denied the workspace mutation.', 403)
  }
  if (code === E.ENAMETOOLONG) return issue('INVALID_PATH', 'The workspace path is too long.', 400)
  if (code === E.ENOSPC) return issue('INSUFFICIENT_STORAGE', 'The Host has insufficient storage.', 507)
  if (code === E.EISDIR || code === E.EINVAL) return issue('INVALID_PATH', 'The workspace path is invalid.', 400)
  return issue('WORKSPACE_MUTATION_FAILED', 'The Host could not prepare the workspace mutation.', 409)
}

function nodeIssue(error: unknown): MutationBackendIssue {
  const code = typeof error === 'object' && error !== null && 'code' in error
    ? String((error as { code?: unknown }).code)
    : undefined
  if (code === 'EEXIST') return issue('DESTINATION_EXISTS', 'The destination already exists.', 409)
  if (code === 'ENOENT') return issue('NOT_FOUND', 'A workspace parent directory no longer exists.', 404)
  if (code === 'ENOTDIR') return issue('NOT_DIRECTORY', 'A workspace parent path is not a directory.', 400)
  if (code === 'ELOOP') return issue('SYMLINK_NOT_ALLOWED', 'Symbolic links are not available in IDE mutation paths.', 403)
  if (code === 'EACCES' || code === 'EPERM' || code === 'EROFS') {
    return issue('PERMISSION_DENIED', 'The Host denied the workspace mutation.', 403)
  }
  if (code === 'ENAMETOOLONG') return issue('INVALID_PATH', 'The workspace path is too long.', 400)
  if (code === 'ENOSPC') return issue('INSUFFICIENT_STORAGE', 'The Host has insufficient storage.', 507)
  return issue('WORKSPACE_MUTATION_FAILED', 'The Host could not prepare the workspace mutation.', 409)
}

class DarwinKernel implements DarwinMutationKernelPort {
  private readonly library = `dsh-code-ide-darwin-${randomUUID()}`
  private disposed = false

  private constructor(private readonly ffi: FfiRuntime) {}

  static async create(): Promise<DarwinKernel> {
  ...
  async publishNoReplace(root: number, source: string, destination: string): Promise<DarwinNativeResult> {
    return await this.call(
      'renameatx_np',
      [this.ffi.DataType.I32, this.ffi.DataType.String, this.ffi.DataType.I32, this.ffi.DataType.String, this.ffi.DataType.U32],
      [root, source, root, destination, C.RENAME_EXCL | C.RENAME_NOFOLLOW_ANY],
    )
  }

  async unlink(parent: number, name: string, directory: boolean): Promise<DarwinNativeResult> {
    return await this.call(
      'unlinkat',
      [this.ffi.DataType.I32, this.ffi.DataType.String, this.ffi.DataType.I32],
      [parent, name, directory ? C.AT_REMOVEDIR : 0],
    )
  }
  ...
  async assertLocalApfs(fd: number): Promise<void> {
    ...
    if (result.value !== 0) throw new Error(`fstatfs failed with errno ${result.errnoCode}.`)
    ...
    if ((flags & C.MNT_LOCAL) === 0 || filesystem !== 'apfs') {
      throw new IdeHostError(
        'WORKSPACE_MUTATION_UNAVAILABLE',
        'Safe structural mutations require a local APFS workspace on macOS.',
        501,
      )
    }
  }
  ...
  private async call(funcName: string, paramsType: FieldType[], paramsValue: unknown[]): Promise<DarwinNativeResult> {
    if (this.disposed) throw new Error('The Darwin mutation kernel is disposed.')
...
class DarwinMutationWorkspace implements MutationBackendWorkspace {
  ...
  constructor(
    ...
    private readonly rootDev: bigint,
    private readonly rootIno: bigint,
    private readonly kernel: DarwinMutationKernelPort,
    private readonly io: DarwinNodeIoPort,
    private readonly onDispose: (workspace: DarwinMutationWorkspace) => void,
  ) {}
  ...
  private async executeSerial(request: MutationBackendExecution): Promise<MutationBackendOutcome> {
    ...
    const cleanStaging = async (): Promise<boolean> => {
      if (stagingName === undefined || !stagingCreated) return true
    ...
    const poisonRecovery = (): MutationBackendOutcome => {
      this.poisoned = true
      return recovery()
    }
    const cleanBeforeCommit = async (outcome: MutationBackendOutcome): Promise<MutationBackendOutcome> => {
      if (await cleanStaging()) return outcome
      return poisonRecovery()
    }
    try {
      if (request.signal.aborted) return aborted()
      const leaf = segments.at(-1)
      if (leaf === undefined) return notCommitted(issue('INVALID_PATH', 'The workspace path is invalid.', 400))
      ...
      if (!expectedCreatedKind(rootInfo, true) || !expectedCreatedKind(namedRootInfo, true)
        ...
        || !sameIdentity(rootInfo, namedRootInfo)) return poisonRecovery()
      ...
      if (request.signal.aborted) return await cleanBeforeCommit(aborted())
      ...
      if (!sameIdentity(stagedInfo, finalInfo) || !sameIdentity(finalInfo, namedFinalInfo)
        ...
        || !expectedCreatedKind(namedFinalInfo, expectedDirectory)) return poisonRecovery()
      return {
        state: 'committed',
      ...
      if (commitEntered) return poisonRecovery()
      if (stagingCreated) {
        await cleanStaging()
        return poisonRecovery()
      }
      return notCommitted(issue(
        ...
        'The Host could not prepare the workspace mutation.',
        409,
      ))
    } finally {
...
export function createDarwinMutationWorkspaceForTesting(
  options: DarwinMutationWorkspaceTestOptions,
): MutationBackendWorkspace {
  return new DarwinMutationWorkspace(
    options.workspaceId,
    options.canonicalRoot,
    options.rootFd,
    options.rootIdentity.dev,
    options.rootIdentity.ino,
    options.kernel,
    options.io ?? DARWIN_NODE_IO,
    () => {},
  )
}

class DarwinMutationBackend implements MutationBackend {
  readonly descriptor = DARWIN_DESCRIPTOR
  private readonly workspaces = new Set<DarwinMutationWorkspace>()
  private readonly opens = new Set<Promise<MutationBackendWorkspace>>()
  private disposed = false
  private disposePromise: Promise<void> | undefined

  constructor(private readonly kernel: DarwinMutationKernelPort) {}
  ...
  openWorkspace(request: OpenMutationBackendWorkspace): Promise<MutationBackendWorkspace> {
    if (this.disposed) return Promise.reject(new IdeHostError(
      'WORKSPACE_MUTATION_UNAVAILABLE', 'The mutation backend is disposed.', 501,
    ))
  ...
  private async openWorkspaceAdmitted(request: OpenMutationBackendWorkspace): Promise<MutationBackendWorkspace> {
    if (request.signal.aborted) throw new IdeHostError('MUTATION_ABORTED', 'The workspace mutation was cancelled.', 409)
    let rootFd: number
    try {
      rootFd = await openFd(
      ...
      throw new IdeHostError(
        'WORKSPACE_MUTATION_UNAVAILABLE',
        'The Host could not open the workspace root without following symbolic links.',
        501,
        { cause: error },
      )
    }
    try {
      ...
      if (!info.isDirectory() || info.isSymbolicLink()) throw new IdeHostError(
        'WORKSPACE_MUTATION_UNAVAILABLE',
        'The registered workspace root is not a local directory.',
        501,
      )
      ...
      if (request.signal.aborted || this.disposed) throw new IdeHostError(
        'WORKSPACE_MUTATION_UNAVAILABLE', 'Workspace opening was cancelled during Host shutdown.', 501,
      )
      const workspace = new DarwinMutationWorkspace(
        request.workspaceId,
        canonicalRoot,
        rootFd,
        info.dev,
        ...
        this.kernel,
        DARWIN_NODE_IO,
        value => { this.workspaces.delete(value) },
      )
...
export function createDarwinMutationBackendForTesting(kernel: DarwinMutationKernelPort): MutationBackend {
  return new DarwinMutationBackend(kernel)
}

async function probe(backend: DarwinMutationBackend): Promise<void> {
