...
import type { OpenMediaResult, WorkspaceFileService } from './filesystem.js'
...
function singleQueryValue(
  ...
  name: 'workspaceId' | 'path' | 'version',
  required: boolean,
): string | undefined {
  const values = params.getAll(name)
  if (values.length > 1) throw new IdeHostError('INVALID_MEDIA_QUERY', `${name} must appear at most once.`)
  const value = values[0]
  if (value === undefined) {
    if (required) throw new IdeHostError('INVALID_MEDIA_QUERY', `${name} is required.`)
    return undefined
  }
  if (value === '') throw new IdeHostError('INVALID_MEDIA_QUERY', `${name} must not be empty.`)
  return value
}

function mediaQuery(request: IncomingMessage): {
  workspaceId: string
  path: string
  version?: string
} {
...
function headerValue(request: IncomingMessage, name: 'range' | 'if-range'): string | undefined {
...
function byteRange(value: string | undefined, size: number): ByteRange | typeof UNSATISFIABLE | undefined {
  if (value === undefined) return undefined
  const match = /^bytes=(\d*)-(\d*)$/u.exec(value)
  if (match === null || (match[1] === '' && match[2] === '')) return UNSATISFIABLE
  const sizeBig = BigInt(size)
  if (sizeBig === 0n) return UNSATISFIABLE
  ...
  if (start >= sizeBig) return UNSATISFIABLE
...
export function createMediaHandler(files: WorkspaceFileService): MediaHandler {
  return async (request, response) => {
    ...
    let media: OpenMediaResult | undefined
    try {
      ...
      if (range !== undefined) response.setHeader('content-range', `bytes ${String(start)}-${String(end)}/${String(media.sizeBytes)}`)
