...
} from '../shared/workspace-observation.js'
import {
...
function concurrencyConflict(message: string, cause?: unknown): IdeHostError {
  return new IdeHostError('VERSION_CONFLICT', message, 409, cause === undefined ? undefined : { cause })
}

async function lstatDuringVersionedOperation(path: string, message: string): Promise<BigIntStats> {
  try {
    ...
    const code = typeof error === 'object' && error !== null && 'code' in error
      ...
      : undefined
    if (code === 'ENOENT') throw concurrencyConflict(message, error)
    throw error
  }
}

function wireChild(parent: string, name: string): string {
...
function parseObservationTarget(value: unknown): ObservationTarget {
...
function missingSnapshot(target: ObservationTarget): ObservationSnapshot {
  return { path: target.path, kind: target.kind, state: 'missing' }
}

function directChildFingerprint(children: Array<{ name: string; kind: FileKind }>): string {
...
): Promise<{ bytes: Buffer; info: BigIntStats; tooLarge: boolean }> {
  const handle = await open(path, READ_FLAGS)
  try {
    const openedBefore = await handle.stat({ bigint: true })
    if (!openedBefore.isFile()) throw new IdeHostError('NOT_FILE', 'The requested path is not a regular file.')
...
function decodeText(bytes: Buffer): string {
  if (bytes.includes(0)) throw new IdeHostError('NOT_TEXT', 'Binary files cannot be opened in the text editor.', 415)
  try {
...
function decodeTextPrefix(bytes: Buffer): string | undefined {
...
function decodeReadOnlyText(bytes: Buffer, complete: boolean): string | undefined {
  if (!complete) return decodeTextPrefix(bytes)
  try {
    return decodeText(bytes)
  } catch (error) {
    if (error instanceof IdeHostError && error.code === 'NOT_TEXT') return undefined
    throw error
  }
}

function readOnlyPresentation(
  reason: ReadOnlyFilePresentation['reason'],
  info: BigIntStats,
  limitBytes: number,
  content: string,
): ReadOnlyFilePresentation {
  return {
    reason,
...
async function currentFile(path: ResolvedWorkspacePath): Promise<{ version: string; mode: number } | undefined> {
  if (!path.exists) return undefined
  const info = await lstat(path.absolutePath, { bigint: true })
  if (info.isSymbolicLink()) throw new IdeHostError('SYMLINK_NOT_ALLOWED', 'Symbolic links cannot be saved.', 403)
  if (!info.isFile()) throw new IdeHostError('NOT_FILE', 'The requested path is not a regular file.')
  return { version: versionOf(info), mode: Number(info.mode & 0o777n) }
}
...
export class WorkspaceFileService {
  ...
  private readonly activeMediaOpens = new Set<Promise<OpenMediaResult>>()
  private readonly activeMediaHandles = new Set<FileHandle>()
  private disposing = false

  constructor(
    private readonly registry: HostWorkspaceRegistry,
    private readonly options: WorkspaceFileServiceOptions,
    private readonly internals: WorkspaceFileServiceInternals = {},
  ...
  workspaces(maxTerminalSessions: number): { workspaces: WorkspaceSummary[]; maxTerminalSessions: number } {
    return {
      maxTerminalSessions,
  ...
  async list(id: unknown, pathValue: unknown): Promise<{ entries: FileEntry[] }> {
    ...
    if (!targetInfo.isDirectory()) throw new IdeHostError('NOT_DIRECTORY', 'The requested path is not a directory.')
      ...
      for await (const child of directory) {
        if (entries.length >= this.options.maxDirectoryEntries) {
          throw new IdeHostError(
            'DIRECTORY_TOO_LARGE',
            `Directory contains more than ${String(this.options.maxDirectoryEntries)} entries.`,
            413,
          )
        }
        if (isInternalWorkspaceName(child.name)) continue
   ...
   */
  async inspect(id: unknown, targetsValue: unknown): Promise<InspectResponse> {
    const workspace = this.requireWorkspace(id)
    ...
    if (targetsValue.length > maxTargets) {
      throw new IdeHostError(
        ...
        `Inspect accepts at most ${String(maxTargets)} targets.`,
        413,
      )
    }
    ...
    for (const target of targets) {
      ...
      const children: Array<{ name: string; kind: FileKind }> = []
      let directory
      try {
        ...
        for await (const child of directory) {
          ...
          if (visitedDirectoryChildren > maxDirectoryChildren) {
            throw new IdeHostError(
              ...
              `Inspect visits at most ${String(maxDirectoryChildren)} direct directory entries.`,
              413,
            )
          }
          ...
          const kind: FileKind = childInfo.isSymbolicLink()
            ? 'other'
  ...
  async read(id: unknown, pathValue: unknown): Promise<ReadFileResult> {
    const workspace = this.requireWorkspace(id)
    ...
    const { bytes, info, tooLarge } = await boundedRead(
      target.absolutePath,
      this.options.maxFileBytes,
      this.internals.afterOpenedFileRead,
    )
    ...
    const finalInfo = await lstatDuringVersionedOperation(
      target.absolutePath,
    ...
    try {
      ...
      if (!(error instanceof IdeHostError) || error.code !== 'NOT_TEXT') throw error
      return {
        path: relativePath,
        content: '',
        version,
  ...
  async openMedia(
    ...
    if (this.disposing) throw new IdeHostError('FILES_UNAVAILABLE', 'Workspace files are stopping.', 503)
  ...
  private async openMediaAdmitted(
    id: unknown,
    pathValue: unknown,
    expectedVersionValue?: unknown,
  ): Promise<OpenMediaResult> {
    ...
    try {
      const openedBefore = await handle.stat({ bigint: true })
      if (!openedBefore.isFile()) throw new IdeHostError('NOT_FILE', 'The requested path is not a regular file.')
      const pathInfo = await lstatDuringVersionedOperation(
        target.absolutePath,
        'The file changed while the media preview was being opened.',
      )
      const after = await realpath(target.absolutePath)
      if (!isPathInside(root.realPath, after)) {
      ...
      if (pathInfo.size > BigInt(maxMediaBytes)) {
        throw new IdeHostError(
          'MEDIA_TOO_LARGE',
          `Media exceeds the ${String(maxMediaBytes)} byte preview limit.`,
          413,
        )
      }
      ...
      if (!leased) await handle.close().catch(() => {})
    }
  }
  ...
  async write(
    ...
    const workspace = this.requireWorkspace(id)
    const relativePath = parseWorkspacePath(pathValue, { allowRoot: false })
    if (typeof contentValue !== 'string') throw new IdeHostError('INVALID_CONTENT', 'content must be a string.')
    if (contentValue.includes('\0')) throw new IdeHostError('INVALID_CONTENT', 'NUL bytes are not valid editor text.')
  ...
  async dispose(): Promise<void> {
    ...
    while (this.activeMutations.size > 0) await Promise.allSettled([...this.activeMutations])
  }
  ...
  private assertExpectedVersion(current: string | undefined, expected: string | undefined): void {
  ...
  private async publish(
    ...
    try {
      ...
      const committed = await lstatDuringVersionedOperation(
        freshTarget.absolutePath,
        'The file changed before the save could be verified.',
      )
      ...
      if (stagingPresent) await unlink(staging).catch(() => {})
    }
  }
