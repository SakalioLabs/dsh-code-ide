...
interface WorkspaceQueue {
  running: boolean
  readonly items: QueuedOperation[]
}
...
function wireWorkspaceId(workspace: HostWorkspace): string {
...
export class WorkspaceResources {
  ...
  constructor(
    ...
    options: WorkspaceResourcesOptions = {},
  ) {
  ...
  async resolveRoot(workspace: HostWorkspace): Promise<ResolvedWorkspaceRoot> {
  ...
  async runMutation<T>(
    ...
    return await new Promise<T>((resolvePromise, rejectPromise) => {
      let item: QueuedOperation<T>
      let abort: (() => void) | undefined
      if (signal !== undefined) {
        const operationSignal = signal
        abort = (): void => {
            const index = queue.items.indexOf(item as QueuedOperation)
            if (index === -1) return
      ...
      queue.items.push(item as QueuedOperation)
      if (abort !== undefined && signal !== undefined) signal.addEventListener('abort', abort, { once: true })
      this.pump(key, queue)
    })
  }
  ...
  async dispose(): Promise<void> {
    if (!this.accepting && this.pendingCount === 0) return
    this.accepting = false
    for (const [key, queue] of this.queues) {
      const queued = queue.items.splice(0)
      for (const item of queued) {
        item.signal?.removeEventListener('abort', item.abort as () => void)
        this.pendingCount -= 1
  ...
  private pump(key: string, queue: WorkspaceQueue): void {
  ...
  private removeIdleQueue(key: string, queue: WorkspaceQueue): void {
