...
function validateWorkspaceSegment(segment: string): void {
...
export function parseWorkspacePath(value: unknown, options: { allowRoot: boolean }): string {
  if (typeof value !== 'string') throw new IdeHostError('INVALID_PATH', 'path must be a string.')
  ...
  for (const segment of segments) validateWorkspaceSegment(segment)
...
export async function resolveWorkspaceRoot(
  registeredPath: string,
  expectedIdentity?: WorkspaceRootIdentity,
...
async function linuxMountPoints(): Promise<string[]> {
  ...
  try {
    ...
    throw new IdeHostError(
      'MOUNT_POLICY_UNAVAILABLE',
      'The Host cannot prove the workspace mount boundary.',
      501,
      { cause: error },
    )
  }
 ...
 */
export async function assertNoNestedMount(
  ...
  for (const mountPoint of await linuxMountPoints()) {
    if (samePath(mountPoint, root.realPath) || !isPathInside(root.realPath, mountPoint)) continue
...
export async function resolveWorkspacePath(
  ...
  if (wirePath === '') return { absolutePath: root.realPath, relativePath: '', exists: true }
  const segments = wirePath.split('/')
  let cursor = root.realPath
  for (let index = 0; index < segments.length; index += 1) {
    const segment = segments[index]
    if (segment === undefined) throw new IdeHostError('INVALID_PATH', 'Invalid path segment.')
