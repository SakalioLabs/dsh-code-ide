import {
  ...
  type WorkspaceFilesProviderContext,
...
export function workspaceFilesProvider(
  ctx: WorkspaceFilesProviderContext,
  config: WorkspaceFilesProviderConfig,
): void {
  ...
  ctx.effect(() => {
    const withdraw = ctx.provide(WORKSPACE_FILES_CAPABILITY, files) as () => void | Promise<void>
    return async () => {
      await withdraw()
      await files.dispose()
    }
